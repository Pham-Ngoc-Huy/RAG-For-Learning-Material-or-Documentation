...
find-clean:
	...
	@for root in . src tests api docs config temp; do \
		if [ -d "$$root" ]; then \
			echo "Checking $$root/:"; \
			found=$$(find $$root -type d \( -name "__pycache__" -o -name "Icon\r" -o -name ".ipycheckpoints" -o -name ".ipynb_checkpoints" \) 2>/dev/null); \
			if [ -n "$$found" ]; then \
				echo "$$found" | sed 's/^/  /'; \
			else \
				echo "  (none found)"; \
...
clean-root:
	...
	@for root in . src tests api docs config temp; do \
		if [ -d "$$root" ]; then \
			echo "Cleaning $$root/:"; \
			find $$root -type d \( -name "__pycache__" -o -name "Icon\r" -o -name ".ipycheckpoints" -o -name ".ipynb_checkpoints" \) -prune -exec rm -rf {} + 2>/dev/null; \
			echo "  ✓ Done"; \
			echo ""; \
		fi; \
	done
...
clean:
	...
	@find . -type d \( -name "__pycache__" -o -name "Icon\r" -o -name ".ipycheckpoints" -o -name ".ipynb_checkpoints" \) -prune -exec rm -rf {} + 2>/dev/null
