...
class BaseChunker(ABC):
    ...
    def _build_chunk_metadata(self, base_metadata: dict, chunk_index: int, total_chunk: int) -> dict:
        return {**base_metadata, "chunk_index": chunk_index, "total_chunk": total_chunk}
...
class RecursiveChunker(BaseChunker):
    ...
    def _split(self, text: str, sep_index: int = 0) -> list[dict]:
    ...
    def chunk(self, doc: dict) -> list[dict]:
        ...
        return [
            {
                "text": c,
                "metadata": self._build_chunk_metadata(doc["metadata"], i, len(merged)),
            }
...
class MarkDownChunker(BaseChunker):
    ...
    def chunk(self, doc: dict) -> list[dict]:
        ...
        return [
            {
                "text": c,
                "metadata": self._build_chunk_metadata(doc["metadata"], i, len(chunks)),
            }
            for i, c in enumerate(chunks)
        ]
